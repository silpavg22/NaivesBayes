...
def naive_bayes(train, test):
    ...
    if max_prob_label == test[-1]:
        if "prediction" in accuracy_dict:  #storing correct prediction count in accuracy_dict
...
def splitdataandpredict():
    ...
    while (i != len(training)):
        new_training=[]     #list for creating new training set leaving one data from training in each iteration
        test = training[i]
